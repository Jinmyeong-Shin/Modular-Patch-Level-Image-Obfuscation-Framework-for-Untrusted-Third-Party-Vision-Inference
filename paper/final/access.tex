...
\begin{abstract}
\rev{Vision Transformer (ViT)--based inference services increasingly rely on third-party infrastructure, creating a practical need to protect sensitive images before they leave the client. This paper presents a modular patch-level obfuscation framework in which a lightweight client-side transform produces visually degraded inputs and a compact server-side deobfuscator maps them to ViT-compatible embeddings for frozen transformer backbones. The contribution is an incremental engineering integration rather than a new cryptographic primitive: the obfuscator is not a lossless permutation and does not expose a closed-form inverse, while privacy is evaluated empirically under explicit attack models. Across the evaluated classification and retrieval settings, the framework preserves high utility, including CIFAR-10 accuracy of 0.9854 under obfuscation and Flickr30k image-to-text Recall@1 of 0.81. We further report Structural Similarity Index (SSIM) and Peak Signal-to-Noise Ratio (PSNR) under direct and adversarial reconstruction attacks: direct obfuscation gives SSIM 0.0022 and PSNR 10.35 dB, while a conservative white-box L-BFGS optimization attack reaches SSIM 0.4598 and PSNR 17.93 dB but does not recover exact images. In contrast, representative reversible perceptual-encryption approaches based on block and pixel permutations are exactly recoverable under known-key or chosen-plaintext calibration. The framework also includes Merkle tree--based configuration attestation for declared model stacks, with deployment assumptions stated explicitly.}
\end{abstract}

\begin{keywords}
\rev{Privacy-Preserving Deep Learning, Vision Transformer, Image Obfuscation, Cloud Inference}
\end{keywords}
...
To address these limitations, we propose a modular and lightweight privacy-preserving framework that supports secure inference on untrusted or semi-trusted infrastructures such as public clouds. \rev{The framework introduces a non-lossless patch-level channel obfuscation method executed on the client side, which disrupts local visual statistics while preserving the high-level semantic features necessary for machine recognition.}
...
\rev{We evaluate the proposed system across a broader benchmark suite spanning classification, retrieval, detection, anomaly detection, segmentation, and captioning. The strongest results occur in classification and retrieval, while segmentation and captioning are reported as limitations or exploratory results rather than as primary success claims. The lightweight deobfuscators can be trained in under 5 minutes, underscoring the framework's practicality for rapid secure-inference deployment.}

The contributions of this paper can be summarized as follows:
\begin{itemize}
    \item \rev{\textbf{Privacy-preserving image obfuscation and deobfuscation architecture:} We propose a non-lossless, channel-wise patch-level obfuscation scheme applied entirely on the client side. This transformation suppresses low-level visual features while preserving high-level semantics, enabling downstream inference without exposing human-recognizable content. A lightweight deobfuscator reconstructs ViT-compatible embeddings without requiring access to the original image.}
...
\subsection{\rev{Reversible Perceptual Encryption for ViT-Compatible Models}}

\rev{A closely related line of work studies perceptual or learnable image encryption with secret keys, including pixel negative--positive transforms, EtC block scrambling, block-wise orthogonal transforms, ViT block/pixel permutations, patch-local pixel shuffling, restricted random permutation matrices, and disposable-key variants. Table~\ref{tab:reversible-family-map} groups representative studies by the reversible transform family considered in this comparison. These methods are important baselines because they often preserve classification utility on encrypted images. At the same time, many are reversible by construction: under known-key exposure, or under chosen-plaintext calibration when a permutation key is reused, the original image can be recovered exactly. This distinction is consistent with prior ciphertext-only analyses in the same literature, including jigsaw-puzzle attacks that recover visual information from ViT-oriented encrypted images~\cite{chuman2023_jigsaw}. Therefore, our comparison separates ciphertext-only visual privacy from known-key, chosen-plaintext, and white-box reconstruction settings rather than treating all privacy claims as equivalent.}

\begin{table*}[t]
\centering
\caption{\rev{Representative reversible perceptual-encryption families considered in this comparison.}}
\label{tab:reversible-family-map}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{0.31\textwidth} p{0.31\textwidth} p{0.30\textwidth}@{}}
\toprule
\rev{\textbf{Transform family}} & \rev{\textbf{Representative works}} & \rev{\textbf{Exact recovery condition}} \\
\midrule
\rev{Pixel negative/channel shuffle} & \rev{\cite{sirichotedumrong2019pixel}} & \rev{Known key} \\
\rev{EtC/block scrambling} & \rev{\cite{kawamura2020etc,maungmaung2022isotropic,qi2022convmixer_adaptive_permutation,hamano2023jpeg_etc_vit,lin2024convmixer}} & \rev{Known or calibrated key} \\
\rev{Block-wise orthogonal transform} & \rev{\cite{aso2023random_orthogonal_convmixer}} & \rev{Known matrix} \\
\rev{ViT block+pixel permutation} & \rev{\cite{qi2022vit,nagamori2023federated_vit,nagamori2024domain_adaptation,kiya2023reliable_vit}} & \rev{Known or calibrated permutation} \\
\rev{Patch-local pixel shuffle} & \rev{\cite{iijima2022convmixer_model_encryption,kiya2023image_model,jimaging8090233}} & \rev{Known or calibrated permutation} \\
\rev{Restricted random permutation} & \rev{\cite{horio2024privacypreservingvisiontransformerusing}} & \rev{Known or calibrated permutation} \\
\rev{Restricted per-image/key permutation} & \rev{\cite{aso2024disposable,hirose2025no_key_management}} & \rev{Compromised target key} \\
\bottomrule
\end{tabular}
\end{table*}
...
\rev{To defend against this class of adversary, the system should satisfy several practical security requirements. First, it should make input reconstruction difficult under the explicit attack models evaluated in this paper, even when the deobfuscator and backbone are exposed. Second, the inference server should not be able to forge or substitute declared model components without detection. Third, inference results should be bound to the correct inputs and configurations to reduce replay, forgery, or selective-misbehavior risks. Finally, the system should reduce the usefulness of adaptive attacks that exploit structural regularities or statistical consistencies across samples to approximate the obfuscation transformation.}
...
\end{equation}
\rev{After transformation, the framework can permute patches within each channel according to a random permutation map $\pi_c$, and the final channel-wise tensor is further scrambled via a global channel permutation $\rho$. In the experiments reported here, the active client transform is interpreted as patch-wise random linear transformation with channel permutation, while patch permutation remains a configurable extension. The final output $\mathbf{Y}$ is formed by reassembling the transformed patches and applying a $\tanh$ activation to bound the value range:}
\begin{equation}
    \mathbf{Y} = \text{tanh}(\text{PermuteChannels}(\text{Assemble}(\pi_c(\mathbf{Q}_{c,i})), \rho))
\end{equation}
\rev{This construction empirically disrupts spatial and channel-wise low-level statistics while preserving enough task-relevant structure for transformer-based models or ViT-like encoders through learned deobfuscation.} \\
...
\rev{To recover task-relevant representations from obfuscated inputs, we introduce a lightweight, learnable deobfuscator network that approximates the embedding space of a frozen ViT-like vision backbone. The deobfuscator is deployed server-side, decoupled from the obfuscator, and is trained to restore semantic embeddings rather than original pixel values. Therefore, compromise of the server-side model stack does not directly expose a reconstruction decoder for the client's original image.}
...
\rev{The proposed framework is designed for untrusted cloud environments and is built on a three-party architecture consisting of a service provider, a client, and an untrusted third-party inference server. The service provider prepares the system by creating client-side obfuscation maps and corresponding lightweight deobfuscators designed to work with a pre-trained backbone model. These components are designed to suppress low-level visual features while preserving the high-level semantics needed for inference. The client, operating as the secure entry point, applies one of these obfuscation maps to an input image, making it visually unrecognizable before transmitting it to the server. The server, which only hosts the deobfuscator and the main inference model, can then perform its task without receiving the original image or the private transformation process.}
...
\rev{This design provides empirical resistance to image reconstruction under the attack settings evaluated in Section~\ref{sec:comparison}. The obfuscation process is not a lossless permutation and does not expose a closed-form inverse, while the deobfuscator is trained only to recover ViT-compatible embeddings, not to reconstruct the original image. To further reduce repeated-query leakage, the framework supports optional random obfuscation scheduling, where the client selects from a pool of signed obfuscation modules for each request. This introduces structural variation that complicates generalized inversion models, while cryptographic attestation allows the client to verify the declared execution configuration.}
...
\begin{equation}
    (T_j, P_j=\{N_j,N_{j2},...\}, \textrm{Sig}_{root})
\end{equation}
...
\subsection{\rev{Obfuscation as an Empirical Structural Barrier}}
\rev{The channel- and patch-level obfuscator is designed as a fixed, randomized transformation that applies patch-wise linear filters and channel-level scrambling before transmission. These transformations are never trained and are instantiated once per client or per obfuscation pool. Their role is to suppress local spatial coherence and low-level visual statistics while maintaining sufficient task-relevant structure for a downstream model to operate correctly.}

\rev{Unlike autoencoder-based methods, the server-side component is not trained to reconstruct pixels. The deployed deobfuscator maps obfuscated inputs to ViT-compatible embeddings, which reduces the reconstruction attack surface compared with decoder-based pipelines. This is an empirical design claim: the security evaluation is bounded by the attack models and measurements reported in Section~\ref{sec:comparison}, not by a formal cryptographic proof.}

\subsection{\rev{Patch Size Mismatch and Reconstruction Difficulty}}
\rev{The deobfuscator operates at a patch size that is intentionally different from that of the obfuscator. Specifically, the patch sizes $p_{\text{obf}}$ and $p_{\text{dobf}}$ are selected such that neither is a divisor of the other. This deliberate misalignment disrupts direct spatial correspondence between the input and output feature maps, making reconstruction more difficult for optimization-based and learned inverse attacks. Reconstructing the original image requires estimating low-level content across mismatched receptive fields while the deobfuscator aggregates information for semantic embedding recovery rather than pixel restoration.}
...
\rev{The attestation mechanism verifies declared configuration membership and binds a response to the obfuscated input hash and nonce. It does not, by itself, prove faithful hardware execution if an adversarial server can bypass the declared binary. Deployments that require stronger runtime guarantees should combine this mechanism with authenticated transport, key protection, audit logging, or trusted execution support.}

\subsection{\rev{Random Obfuscation Scheduling}}
\rev{To improve empirical resistance against model inversion strategies, the framework optionally supports randomized scheduling of obfuscators at the client side. Instead of applying a fixed transformation to every input, the client selects an obfuscator at random from a predefined pool authorized by the provider. This variation introduces structural uncertainty that disrupts the consistency attackers rely on when attempting to infer inverse mappings from multiple samples.}

\rev{Furthermore, this scheduling contains the impact of a secondary threat where one client's obfuscation parameters are exposed. Other clients can use different, independently initialized obfuscators from the pool, so exposure of one configuration need not reveal the configuration used by others. In this context, the size of the obfuscator pool functions as a practical security parameter that increases attacker uncertainty, while performance remains consistent because the selected obfuscator identity is included in the attested execution tuple $T_j$.}

\subsection{\rev{Empirical Reconstruction Resistance and Scope}}\label{sec:non-inv}

\rev{We do not claim that the obfuscator is a formally proven cryptographic primitive. The system can be modeled as a function that transforms an input image tensor $X \in \mathbb{R}^{C \times H \times W}$ into an obfuscated output tensor $Y \in \mathbb{R}^{C \times H \times W}$ through patch-wise random filtering, channel scrambling, and a bounded nonlinear activation. This transform is not a lossless permutation and does not provide the simple closed-form inverse available to reversible block or pixel permutation schemes.}

\rev{The resulting inverse problem combines blind estimation of the client-side transform with reconstruction from a representation optimized for downstream semantic embeddings rather than pixels. The $\tanh$ activation further bounds the output range and can saturate low-level magnitude information, which makes exact recovery ill-posed in practice. We therefore evaluate reconstruction resistance empirically with direct observation, learned VAE and CycleGAN inversion, MI-FGSM perturbation, and a conservative L-BFGS optimization attack with white-box access to the evaluated transform.}

\subsection{\rev{Search-Space Discussion of Patch-Level Obfuscation}}

\rev{To characterize the search space faced by an attacker without the client-side configuration, we estimate the number of possible patch-level transformations. The obfuscator applies randomized kernels to non-overlapping image patches and may include channel and patch permutations depending on the selected configuration. Let the input image of size $224 \times 224$ be divided into patches of size $16 \times 16$, resulting in $N = 196$ patches per channel. Across $C = 3$ channels and $P$ patch groups, the search-space contribution from patch-level transformations is approximately $C \cdot P \cdot \log_2(256!)$ bits. Spatial patch permutations within each channel contribute up to $C \cdot \log_2(N!)$ bits when enabled, and global channel permutation introduces $\log_2(C!)$ bits.}
...
\end{equation}
\rev{Applying Stirling's approximation, this estimate exceeds $10^5$ bits when $P \geq 16$ under the full permutation setting. This value should be read as a search-space heuristic rather than a cryptographic security proof. In practice, reconstruction resistance is established by the empirical attacks reported below, while patch-size mismatch and channel-level scrambling increase the uncertainty faced by attackers that do not possess the client-side configuration.}
...
\rev{For reproducibility, the implementation, experiment scripts, and all experimental artifacts used to produce the reported results are available in the public repository: \url{https://github.com/Jinmyeong-Shin/Modular-Patch-Level-Image-Obfuscation-Framework-for-Untrusted-Third-Party-Vision-Inference}.}
...
\begin{table*}[t]
\centering
\caption{\rev{Benchmark performance across tasks with and without obfuscation.}}
\label{tab:performance}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.13\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.22\textwidth} c c c}
\toprule
\rev{\textbf{Task}} & \rev{\textbf{Dataset}} & \rev{\textbf{Model}} & \rev{\textbf{Metric}} & \rev{\textbf{Clean}} & \rev{\textbf{Obfuscated}} & \rev{\textbf{Change}} \\
\midrule
\rev{Classification} & \rev{CIFAR-10} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.9858} & \rev{0.9854} & \rev{$-0.0004$} \\
\rev{Classification} & \rev{CIFAR-100} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.9176} & \rev{0.9133} & \rev{$-0.0043$} \\
\rev{Classification} & \rev{PathMNIST} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.9428} & \rev{0.9373} & \rev{$-0.0054$} \\
\rev{Classification} & \rev{DermaMNIST} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.8025} & \rev{0.7805} & \rev{$-0.0219$} \\
\rev{Classification} & \rev{BloodMNIST} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.9798} & \rev{0.9626} & \rev{$-0.0172$} \\
\rev{Classification} & \rev{ImageNet-1K} & \rev{ViT-B/16} & \rev{Accuracy} & \rev{0.7126} & \rev{0.6586} & \rev{$-0.0540$} \\
\midrule
\rev{Zero-shot classification} & \rev{CIFAR-10} & \rev{CLIP-B/32} & \rev{Accuracy} & \rev{0.8738} & \rev{0.8500} & \rev{$-0.0238$} \\
\rev{Image retrieval} & \rev{CIFAR-10} & \rev{CLIP-B/32} & \rev{Recall@1} & \rev{0.8870} & \rev{0.8810} & \rev{$-0.0060$} \\
\rev{Image-text retrieval} & \rev{Flickr30k} & \rev{CLIP-B/32} & \rev{Image-to-text Recall@1} & \rev{0.8400} & \rev{0.8100} & \rev{$-0.0300$} \\
\rev{Image-text retrieval} & \rev{Flickr30k} & \rev{CLIP-B/32} & \rev{Text-to-image Recall@1} & \rev{0.7950} & \rev{0.7700} & \rev{$-0.0250$} \\
\midrule
\rev{Object detection} & \rev{COCO 2017} & \rev{YOLOS-Tiny} & \rev{mAP} & \rev{0.2709} & \rev{0.2329} & \rev{$-0.0380$} \\
\rev{Zero-shot detection} & \rev{COCO 2017} & \rev{OWLViT-B/32} & \rev{mAP} & \rev{0.2470} & \rev{0.2144} & \rev{$-0.0326$} \\
\rev{Anomaly detection} & \rev{MVTec AD} & \rev{CLIP-B/32} & \rev{Image AUROC} & \rev{1.0000} & \rev{0.7168} & \rev{$-0.2832$} \\
\rev{Anomaly localization} & \rev{MVTec AD} & \rev{CLIP-B/32} & \rev{Pixel AUROC} & \rev{0.9694} & \rev{0.9414} & \rev{$-0.0280$} \\
\midrule
\rev{Medical segmentation} & \rev{miniMSD} & \rev{ViT-B/16} & \rev{Dice (limitation)} & \rev{0.0000} & \rev{0.0000} & \rev{0.0000} \\
\rev{Captioning} & \rev{Flickr30k} & \rev{BLIP-base} & \rev{BLEU-4 (exploratory)} & \rev{0.1007} & \rev{0.1113} & \rev{$+0.0106$} \\
\bottomrule
\end{tabular}
\end{table*}
...
To complement visual inspection, we compute Structural Similarity Index~(SSIM) and Peak Signal-to-Noise Ratio~(PSNR) between the original and obfuscated images. As shown in Table~\ref{tab:obf-metrics}, both metrics decrease monotonically with larger patch group sizes. SSIM values fall well below 0.1, indicating that perceptual resemblance to the original images is minimal regardless of configuration.
...
\rev{\textbf{Proposed}} & \rev{ViT-B/16}    & \rev{\textbf{0.9854}} & \rev{\textbf{0.9133}} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[t!]
    \centering
    \includegraphics[width=\textwidth]{figures/one_to_one_reconstruction_comparison.png}
    \caption{\rev{One-to-one qualitative reconstruction comparison on the same CIFAR-10 samples. Direct observation compares the proposed obfuscated output with a representative reversible permutation baseline. Paired learned inversion compares VAE and CycleGAN reconstructions for both methods. In the known-key recovery columns, exact inversion is not applicable to the proposed method because it has no reversible key inverse, while the reversible baseline recovers the original exactly when the key is known.}}
    \label{fig:one_to_one_reconstruction}
\end{figure*}


\rev{To assess practical utility and generalization, we evaluate the framework across multiple task families under both standard and obfuscated settings. Table~\ref{tab:performance} summarizes the current benchmark suite using the primary metric for each task, together with the absolute change induced by obfuscation.}

\rev{The strongest results occur in classification and retrieval. CIFAR-10 and CIFAR-100 top-1 accuracy decrease by only 0.0004 and 0.0043, respectively, and CIFAR-10 image retrieval loses only 0.0060 in Recall@1. Flickr30k image-text retrieval remains usable, with image-to-text and text-to-image Recall@1 decreases of 0.0300 and 0.0250. Spatially sensitive tasks show larger but bounded degradation: YOLOS-Tiny mAP decreases from 0.2709 to 0.2329, and OWLViT zero-shot detection mAP decreases from 0.2470 to 0.2144.}

\rev{The broader benchmark suite also clarifies limitations. MVTec anomaly detection preserves pixel-level AUROC comparatively well, decreasing from 0.9694 to 0.9414, but image-level AUROC drops from 1.0000 to 0.7168. The miniMSD segmentation run fails for both clean and obfuscated inputs under the evaluated setup, so it is reported as a limitation rather than evidence of successful segmentation. The BLIP captioning row is included as exploratory because the absolute BLEU scores are low and should not be interpreted as a strong captioning result.}
...
\rev{Table~\ref{tab:accuracy-comparison} summarizes the accuracy results for all compared methods. In our CIFAR experiments, the proposed method achieves 0.9854 top-1 accuracy on CIFAR-10 and 0.9133 on CIFAR-100, maintaining high task utility under client-side obfuscation. The reversible permutation-based baseline remains competitive for classification, but utility alone does not characterize privacy: its transform is exactly recoverable when the corresponding key or a calibrated permutation is available.}

\rev{In contrast, the proposed method uses a non-lossless channel-wise patch-level transform combined with a lightweight, independently trained deobfuscator. This strategy preserves semantic information required for high task accuracy without exposing a pixel reconstruction decoder on the server. Moreover, our framework requires no retraining of existing pretrained ViT backbones, facilitating straightforward and efficient integration into established vision workflows.}

\subsection{\rev{Empirical Resistance to Reconstruction Attacks}}



\rev{To evaluate reconstruction exposure, we use our CIFAR-10 attack evaluation and separate the threat settings that are often conflated in visual privacy comparisons. Figure~\ref{fig:one_to_one_reconstruction} uses the same samples for every column, so direct observation, learned inversion, and known-key recovery are compared one-to-one. The reversible baseline is visually obfuscating under ciphertext-only observation, but it recovers the original exactly when the key or calibrated permutation is available.}

\rev{For the proposed method, direct observation produces SSIM 0.0022 and PSNR 10.35 dB. Learned VAE and CycleGAN attacks remain low-structure reconstructions, with SSIM 0.0656 and 0.0438, respectively. The L-BFGS result should be interpreted as a conservative white-box optimization stress test rather than a known-key inverse or a formal proof: it reaches SSIM 0.4598 without exact recovery.}

\rev{We also evaluated simple distributional leakage indicators on the obfuscated outputs: frequency correlation 0.0019, spatial autocorrelation $-0.1885$, histogram KL divergence 2.0184, and mutual information 0.0407. These values do not constitute a formal side-channel proof, but they document the low-level leakage channels tested in our evaluation.}

\begin{table*}[t!]
\centering
\caption{\rev{Reconstruction metrics for the proposed method on CIFAR-10.}}
\label{tab:proposed_attack_metrics}
\begin{tabular}{p{0.26\textwidth} p{0.36\textwidth} c c c}
\toprule
\rev{\textbf{Attack setting}} & \rev{\textbf{Adversary capability}} & \rev{\textbf{SSIM}} & \rev{\textbf{PSNR}} & \rev{\textbf{MSE}} \\
\midrule
\rev{Direct obfuscated output} & \rev{Ciphertext-only visual observation} & \rev{0.0022} & \rev{10.35} & \rev{0.3694} \\
\rev{MI-FGSM} & \rev{Iterative gradient perturbation attack} & \rev{0.1192} & \rev{11.50} & \rev{0.2830} \\
\rev{L-BFGS} & \rev{White-box optimization stress test} & \rev{0.4598} & \rev{17.93} & \rev{0.0645} \\
\rev{Adversarial VAE} & \rev{Learned inverse model} & \rev{0.0656} & \rev{11.61} & \rev{0.2764} \\
\rev{CycleGAN} & \rev{Unpaired generative reconstruction} & \rev{0.0438} & \rev{10.40} & \rev{0.3645} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[t!]
\centering
\caption{\rev{Direct observation and known-key/chosen-plaintext recovery comparison for reversible transform families.}}
\label{tab:reversible_recovery_metrics}
\begin{tabular}{p{0.30\textwidth} c c c p{0.26\textwidth}}
\toprule
\rev{\textbf{Transform family}} & \rev{\textbf{Direct SSIM}} & \rev{\textbf{Direct PSNR}} & \rev{\textbf{Direct MSE}} & \rev{\textbf{Known-key or calibrated recovery}} \\
\midrule
\rev{Proposed obfuscator} & \rev{0.0022} & \rev{10.35} & \rev{0.3694} & \rev{No closed-form exact inverse} \\
\rev{PE pixel negative/channel shuffle} & \rev{0.0076} & \rev{8.41} & \rev{0.5774} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{EtC block scrambling} & \rev{0.0281} & \rev{8.43} & \rev{0.5742} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{Block orthogonal secret key} & \rev{0.0009} & \rev{8.32} & \rev{0.5891} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{ViT block+pixel permutation} & \rev{0.0279} & \rev{8.55} & \rev{0.5590} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{Patch-local pixel shuffle} & \rev{0.1462} & \rev{13.63} & \rev{0.1734} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{Restricted random permutation} & \rev{0.2653} & \rev{12.22} & \rev{0.2400} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\rev{Restricted per-image/key permutation} & \rev{0.3956} & \rev{14.00} & \rev{0.1594} & \rev{SSIM 1.0, PSNR $\infty$, MSE 0} \\
\bottomrule
\end{tabular}
\end{table*}

\rev{Table~\ref{tab:reversible_recovery_metrics} reports the complementary reversible-family comparison. The intended point is narrow: reversible perceptual-encryption schemes can provide useful visual obfuscation under ciphertext-only settings, but exact recovery follows when the relevant key or calibrated permutation is available. The proposed method is evaluated instead as a non-lossless empirical obfuscator whose residual reconstruction exposure is measured by the attacks in Table~\ref{tab:proposed_attack_metrics}.}
...
\rev{To evaluate the practical viability of the proposed framework, we analyze the computational costs introduced by the client-side Obfuscator and the server-side Deobfuscator. The results, summarized in Table~\ref{tab:overheads}, show that the proposed approach provides empirical reconstruction resistance with manageable and well-defined overhead.}
...
\rev{This paper presents a modular framework for privacy-preserving vision inference designed for deployment in untrusted computation environments. By decoupling image obfuscation from model inference and avoiding server-side image reconstruction, the proposed approach provides empirical resistance to the reconstruction attacks evaluated in this work while maintaining compatibility with existing transformer-based vision models. The channel-wise patch-level obfuscator, paired with a lightweight deobfuscator network, enables accurate inference from visually anonymized inputs. We further reinforce execution integrity through Merkle tree--based attestation and optional randomized obfuscation scheduling. Experimental results across classification, retrieval, detection, and anomaly-detection tasks support the practicality of the design, while the reconstruction tables clarify its attack-bounded security scope.}

\rev{While our method shows good utility under visual obfuscation, it has limitations in the number of model parameters. In particular, the deobfuscator, though efficient to train, requires a separate set of projection layers per output patch, which may introduce scalability challenges for high-resolution inputs or dense downstream tasks. These limitations will be addressed in future work.}
