...
\begin{abstract}
Transformer (ViT)–based architectures. The proposed framework introduces a modular patch-level image obfuscation scheme, where client-side patch-level transformations convert input images into visually unrecognizable forms while preserving the semantic structure required by ViT patch embeddings. These obfuscated images retain high-level semantics required for recognition while ensuring non-invertibility, even when untrusted inference servers are compromised. To support compatibility with ViT-based architectures, the framework decouples obfuscation from recognition by introducing a lightweight deobfuscator network, that approximates ViT-compatible embeddings directly from obfuscated inputs. This decoupling design on untrusted third-party infrastructure eliminates the need for raw image reconstruction at the service end and thus, enables seamless integration with frozen transformer backbone. Also, the proposed framework incorporates a Merkle tree–based attestation mechanism to ensure execution integrity and supports randomized obfuscation scheduling to defend resilience against various reconstruction attacks. Experimental results demonstrate high utility, achieving over 95\% top-1 accuracy on CIFAR-10 classification and over 92\% word-level F1-score on OCR tasks. As each deobfuscator is trained in less than 5 minutes, these results confirm the framework’s efficiency and its suitability for real-world secure vision applications requiring minimal training time and rapid deployment.
\end{abstract}

\begin{keywords}
Privaccy-Preserving Deep Learning, Vision Transformer, Data Privacy, Cloud Computing Environment
\end{keywords}
...
We evaluate the proposed system across a range of vision tasks using ViT-based models and demonstrate that it maintains high utility, achieving over 95\% top-1 accuracy on CIFAR-10 and over 92\% word-level F1 score on OCR benchmarks. The lightweight deobfuscators can be trained in under 5 minutes, underscoring the framework’s practicality and readiness for real-world secure inference applications.
...
\subsection{Theoretical Guarantees of Non-Invertibility}
...
& \multirow{2}{*}{} & \multirow{2}{*}{YOLOS-B/16} & \multirow{2}{*}{800} 
& --     & --     &  &    &      &      \\
&       &        &        & 14x14 & 16x16 &  &  &  & 15  \\
\bottomrule
\end{tabular}
...
Classification on CIFAR-10 and CIFAR-100 shows that the performance degradation due to obfuscation is relatively modest when patch sizes are sufficiently fine-grained. For instance, ViT-B/16 and ViT-L/16 on CIFAR-10 retain $99.13\%$ and $99.37\%$ top-1 accuracy respectively with obfuscation, representing less than $1.5\%$ degradation compared to the original baselines. Similar trends are observed on CIFAR-100, where ViT-L/16 retains over $91\%$ accuracy under obfuscation.

Zero-shot classification using CLIP also demonstrates robustness under the proposed obfuscation. CLIP-B/16 and CLIP-L/14 maintain strong performance on both CIFAR-10 and CIFAR-100, with only a $1.5$–$3.0\%$ absolute drop in top-1 accuracy when switching from clean input to obfuscated input followed by deobfuscation. Notably, the patch sizes between the obfuscator and deobfuscator are intentionally mismatched (e.g., $14\times14$ vs. $16\times16$), contributing to increased privacy while preserving classification performance.
...
{We further evaluate the framework on the complex task of object detection using the YOLOS-B/16 model on the COCO 2017 dataset. As expected, this spatially sensitive task is more affected by obfuscation than classification. At a higher input resolution of 384x384, the model shows significant resilience, with the mAP@0.5 only decreasing from 0.4674 to 0.4067. This indicates that with sufficient input resolution, our method can preserve the features necessary for object localization. A closer look reveals that performance degradation is most pronounced for small objects, mAP-s, while the model remains highly effective for medium and large objects, mAP-m and mAP-l, which is consistent with the nature of patch-level disruptions.
...
This paper presents a new modular framework for privacy-preserving vision inference designed for deployment in untrusted computation environments. By decoupling image obfuscation from model inference and eliminating the need for image reconstruction, the proposed approach ensures strong privacy guarantees while maintaining compatibility with existing transformer-based vision models. The channel-wise patch-level obfuscator, paired with a lightweight deobfuscator network, enables accurate inference from visually anonymized inputs. We further reinforce the system’s security through Merkle tree–based attestation and randomized obfuscation scheduling. Experimental results across diverse tasks confirm the practicality and robustness of our design, with minimal performance loss and substantial resilience to inversion and tampering attacks.

While our method showed good performance under strong privacy constraints, it has limitations in the number of model parameters. In particular, the deobfuscator, though efficient to train, requires a separate set of projection layers per output patch, which may introduce scalability challenges for high-resolution inputs or dense downstream tasks. These limitations will be addressed in future work.
